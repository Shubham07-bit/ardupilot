        ...
        AP_HAL::OwnPtr<AP_HAL::I2CDevice> dev)
{
    ...
    AP_RangeFinder_Benewake_TFS20L *sensor = NEW_NOTHROW AP_RangeFinder_Benewake_TFS20L(_state, _params, std::move(dev));
...
void AP_RangeFinder_Benewake_TFS20L::update()
{
    ...
    if (total_count > 0) {
        if (accum.out_of_range_count == total_count) {
            ...
            state.distance_m = max_distance_cm() * 0.01f + (BENEWAKE_OUT_OF_RANGE_ADD_CM * 0.01f);
